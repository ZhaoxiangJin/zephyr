...
loader API or the MCUmgr eBPF group.
...
and loaded through :c:func:`ebpf_loader_load` or the MCUmgr eBPF management
group. They are the right choice for signed field diagnostics, temporary
investigation probes, and TTL-limited instrumentation.
...
* a runtime loader with image authentication, bundle TTL, and named bundle
  control,
* MCUmgr operations for load, enable, disable, unload, dump, and map read.
...
4. Load the bundle with :c:func:`ebpf_loader_load` or stage it through the
   MCUmgr eBPF group.
