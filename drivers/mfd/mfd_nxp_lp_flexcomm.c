 ...
 * (writes PSELID[PERSEL]). This ungates the selected module's register clock;
 * it must be (re)done at init and after any event that resets the wrapper
 * (e.g. exiting a low-power state that collapses the wrapper's power domain),
 * otherwise accesses to the child module's registers bus-fault.
 */
