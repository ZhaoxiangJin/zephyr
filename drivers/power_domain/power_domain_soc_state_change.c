...
static int pd_pm_action(const struct device *dev, enum pm_device_action action)
{
	...
	case PM_DEVICE_ACTION_SUSPEND:
		...
		/*
		 * Power only goes away for the states the node lists. Without
		 * CONFIG_PM there is no state machine to ask and
		 * pm_state_next_get() is a stub that returns NULL. The domain
		 * device still has to exist, because peripherals name it in
		 * power-domains and that reference has to resolve, so answer the
		 * action without visiting them.
		 */
