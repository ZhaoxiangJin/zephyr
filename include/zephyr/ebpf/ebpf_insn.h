...
/**
 * @file
 * @brief eBPF instruction definitions and construction helpers.
 * @ingroup ebpf
 */
...
#include <zephyr/ebpf/ebpf_insn_basic.h>
...
/** @brief Architectural eBPF register numbers. */
 ...
 *
 * @param[in] _op Encoded eBPF opcode.
 * @param[in] _dst Destination register number.
 * @param[in] _src Source register number.
 * @param[in] _off Signed instruction offset.
 * @param[in] _imm Signed immediate value.
 */
...
/** @brief Return the destination register operand encoded in one instruction. */
#define EBPF_INSN_DST(insn)	((insn)->regs & 0x0FU)
/** @brief Return the source register operand encoded in one instruction. */
