...
The sample does not configure any PMC register masks. The SoC low-power entry
path decides what has to stay powered across a Compute-domain window from
``CONFIG_SECOND_CORE_MCUX``: with the Sense core in the build it keeps the
SENSE/COMN main clocks, the shared oscillators, VDDN_COM and all main-memory
partitions alive, so CPU1 keeps running in cases 5-7 and both cores resume in
place in case 8. All the application does is coordinate over the MU and gate the
deeper states with ``pm_policy_state_lock_get()``/``pm_policy_state_lock_put()``.
