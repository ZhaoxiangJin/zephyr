...
This sample profiles CPU utilisation by attaching two hand-crafted eBPF
programs to the ``IDLE_ENTER`` and ``IDLE_EXIT`` tracing targets. Every time
the CPU leaves the idle state a 49-instruction program — written entirely
with the eBPF instruction macros — runs five phases in a single invocation:

- it calls ``ktime_get_ns`` to compute the idle duration,
- accumulates total idle nanoseconds in a statistics map,
- increments a transition counter in the same map,
- classifies the duration into one of six histogram buckets via a chain of ``JLT``
  (jump-if-less-than) comparisons, and
- increments the matching bucket in a histogram map.

A companion 11-instruction program timestamps entry into the idle thread using
``ktime_get_ns`` and stores it in a shared array map.

Together the three cooperating array maps (timestamp, statistics, histogram)
let the application compute delta-based reports every two seconds showing
active/idle percentage, transition count, and an ASCII histogram of idle
durations ranging from *< 100 µs* to *> 100 ms*. A worker thread switches
to a different sleep duration each report interval so that the dominant
histogram bucket visibly shifts from report to report. Tickless kernel mode
is enabled to minimise background timer noise.
...
.. code-block:: none
   ...
   main()
     ...
     |-- attach idle_enter --> IDLE_ENTER target
     |-- attach idle_exit  --> IDLE_EXIT  target
     |-- enable both programs (triggers verifier once per program)
     |-- spawn worker thread
...
.. code-block:: none
   ...
   Zephyr kernel
     ...
     |       |
     |       '--> idle_enter program (11 insns)
     |               ts = ktime_get_ns()
     |               ts_map[0] = ts
     |
     '-- CPU wakes up --> sys_trace_idle_exit()
           |
           '--> idle_exit program (49 insns)
...
.. code-block:: console
   ...
   *** Booting Zephyr OS build v4.3.0 ***
   ...
   eBPF programs enabled on IDLE_ENTER / IDLE_EXIT.
