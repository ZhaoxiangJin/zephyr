 ...
 *
 * Two eBPF programs cooperate on the IDLE_ENTER / IDLE_EXIT tracing targets to
 * build an in-kernel profile of CPU utilisation:
 *
 *  - idle_enter — timestamps entry into the idle thread.
 *  - idle_exit  — 49-instruction eBPF program that
 ...
 *
 * Five phases executed on every idle-to-active transition:
 *
 ...
 *   key = 0;
 *   uint64_t *total = map_lookup_elem(&stats_map, &key);
 *   if (total) { *total += duration; }
 *
 *   // count idle-to-active transitions
 *   key = 1;
 *   uint64_t *count = map_lookup_elem(&stats_map, &key);
 *   if (count) { *count += 1; }
 *
 *   // classify into histogram bucket
 *   uint32_t bucket = 0;
 *   if (duration >= 100000)    bucket = 1;
 *   if (duration >= 500000)    bucket = 2;
 *   if (duration >= 1000000)   bucket = 3;
 *   if (duration >= 10000000)  bucket = 4;
 *   if (duration >= 100000000) bucket = 5;
 *
 *   // increment histogram[bucket]
 *   uint64_t *slot = map_lookup_elem(&histogram, &bucket);
 *   if (slot) { *slot += 1; }
 *   return 0;
 * @endcode
 */
static struct ebpf_insn idle_exit_code[] = {
	...
	/* [11] */
	EBPF_ADD64_IMM(EBPF_REG_R2, -4),
	/* [12] R1 = stats_map handle (patched) */
	EBPF_MOV64_IMM(EBPF_REG_R1, 0),
	...
	/* [20] */
	EBPF_ADD64_IMM(EBPF_REG_R2, -4),
	/* [21] R1 = stats_map handle (patched) */
	EBPF_MOV64_IMM(EBPF_REG_R1, 0),
	...
	/* [40] */
	EBPF_ADD64_IMM(EBPF_REG_R2, -4),
	/* [41] R1 = histogram handle (patched) */
	EBPF_MOV64_IMM(EBPF_REG_R1, 0),
...
/*  Programs                                                          */
/* ------------------------------------------------------------------ */
EBPF_PROG_DEFINE(idle_enter, EBPF_PROG_TYPE_GENERIC,
		 idle_enter_code, ARRAY_SIZE(idle_enter_code));

EBPF_PROG_DEFINE(idle_exit, EBPF_PROG_TYPE_GENERIC,
...
int main(void)
{
	...
	idle_enter_code[5].imm  = (int32_t)ebpf_map_get_id(&ts_map);

	idle_exit_code[5].imm   = (int32_t)ebpf_map_get_id(&ts_map);
	idle_exit_code[12].imm  = (int32_t)ebpf_map_get_id(&stats_map);
	idle_exit_code[21].imm  = (int32_t)ebpf_map_get_id(&stats_map);
	idle_exit_code[41].imm  = (int32_t)ebpf_map_get_id(&histogram);

	/* ---- Attach and enable ---- */
	ret = ebpf_prog_attach(&idle_enter,
	...
	ret = ebpf_prog_attach(&idle_exit,
	...
	printk("eBPF programs enabled on IDLE_ENTER / IDLE_EXIT.\n");
	...
	for (int i = 0; i < NUM_REPORTS; i++) {
		...
		/* --- Print report --- */
		printk("\n--- CPU Profiler [%d] %s ---\n", i + 1, phase_labels[i]);
		printk("Active: %u%%  Idle: %u%%  Transitions: %llu\n",
