 ...
 * may produce inaccurate results for the outer ISR.
 */
...
EBPF_PROG_DEFINE(isr_entry, EBPF_PROG_TYPE_ISR,
		 isr_entry_code, ARRAY_SIZE(isr_entry_code));

EBPF_PROG_DEFINE(isr_exit, EBPF_PROG_TYPE_ISR,
...
int main(void)
{
	 ...
	 */
	isr_entry_code[5].imm = (int32_t)ebpf_map_get_id(&ts_map);
	isr_exit_code[5].imm = (int32_t)ebpf_map_get_id(&ts_map);
	isr_exit_code[11].imm = (int32_t)ebpf_map_get_id(&events_map);

	/* Attach and enable the entry program */
	ret = ebpf_prog_attach(&isr_entry, EBPF_ATTACH_TARGET_TRACING(EBPF_TRACING_ATTACH_ISR_ENTER));
	if (ret) {
	...
	printk("eBPF programs enabled on ISR_ENTER and ISR_EXIT.\n");
	...
	for (int i = 0; i < 10; i++) {
		...
		if (count > 0) {
			printk("[%2d] ISRs: %u  avg: %llu ns  min: %llu ns  max: %llu ns\n",
			       i + 1, count, total_ns / count, min_ns, max_ns);
		} else {
