...
#if DT_NODE_HAS_PROP(CPU_FREQ_THERMAL_CAP_NODE, fail_safe_pstate)
#define CPU_FREQ_THERMAL_CAP_FAIL_SAFE_PSTATE						\
	PSTATE_DT_GET(DT_PHANDLE(CPU_FREQ_THERMAL_CAP_NODE, fail_safe_pstate))
#else
#define CPU_FREQ_THERMAL_CAP_FAIL_SAFE_PSTATE NULL
#endif
...
static const struct pstate *const thermal_pstates[] = {
...
static const struct pstate *const fail_safe_pstate = CPU_FREQ_THERMAL_CAP_FAIL_SAFE_PSTATE;
...
static void thermal_cap_handle_error(int error)
{
	...
	LOG_WRN("Thermal sensor sample failed: %d", error);
...
{
	for (size_t i = 0; i < ARRAY_SIZE(thermal_pstates); i++) {
		if (thermal_pstates[i] == state) {
			*index = i;
			return 0;
		}
	}
...
static void thermal_cap_update(int32_t temperature_mc)
{
	...
	for (size_t i = 0; i < ARRAY_SIZE(thermal_trips); i++) {
		...
		/* If the trip is currently inactive and temperature >= trip threshold,
		 * then activate it.
		 */
		if (!trip->active && temperature_mc >= trip->temperature_mc) {
			trip->active = true;
		}

		/* If the trip is currently active and temperature <= release temperature,
		 * then release it.
		 */
...
const struct pstate *cpu_freq_thermal_cap_apply(const struct pstate *state)
{
	...
	if ((cap_index < 0) || ((size_t)cap_index >= ARRAY_SIZE(thermal_pstates))) {
		LOG_ERR("Invalid thermal cap index %d", (int)cap_index);
		return state;
	}

	return thermal_pstates[MAX(requested_index, (size_t)cap_index)];
}
 ...
 * When there is an active trip, use the passive polling interval, which is usually shorter,
 * so that the temperature can be monitored more frequently.
 */
static k_timeout_t thermal_cap_next_timeout(void)
{
	if (atomic_get(&trip_active) != 0) {
		return K_MSEC(DT_PROP(CPU_FREQ_THERMAL_CAP_NODE, passive_polling_delay_ms));
	}
...
static int thermal_cap_init(void)
{
	...
	/*
	 * If fail-safe-pstate is configured in DTS, check whether it exists in the P-state table.
	 * If not configured, use the lowest performance P-state by default.
	 */
	if (fail_safe_pstate != NULL) {
		ret = thermal_cap_pstate_index(fail_safe_pstate, &fail_safe_index);
		if (ret != 0) {
			LOG_ERR("Thermal cap fail-safe P-state is not "
				"in the CPUFreq P-state table");
			return ret;
		}
	} else {
		fail_safe_index = ARRAY_SIZE(thermal_pstates) - 1U;
	}
