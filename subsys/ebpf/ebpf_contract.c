...
/* All current attachment paths expose the same minimal helper set. */
#define EBPF_HELPERS_COMMON (EBPF_HELPER_MASK(EBPF_HELPER_MAP_LOOKUP_ELEM) |	\
...
#define EBPF_TARGET_NONE_ENTRY(_type)						\
	{									\
		...
		.helper_mask = EBPF_HELPERS_COMMON,				\
		.ctx_read_only = true,						\
	}
