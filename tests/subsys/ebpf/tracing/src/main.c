...
static void loader_build_dynamic_map_image(struct loader_test_image *image)
{
	...
	image->attachment.prog_type = EBPF_PROG_TYPE_GENERIC;
...
static void loader_build_ttl_image(struct loader_test_image *image,
				    uint32_t map_id, uint32_t ttl_ms)
{
	size_t strings_len = 0U;
	...
	image->header.header_size = sizeof(image->header);
	image->header.auth_type = EBPF_LOADER_AUTH_CRC32;
	image->header.ttl_ms = ttl_ms;
	image->header.map_count = 0U;
	image->header.attachment_count = 1U;
	image->header.reloc_count = 0U;
	image->header.maps_offset = 0U;
	image->header.attachments_offset = offsetof(struct loader_test_image, attachment);
	image->header.relocs_offset = 0U;
	image->header.strings_offset = offsetof(struct loader_test_image, strings);
	image->header.bundle_name_offset = loader_image_add_string(image, &strings_len,
							  "loader_ttl_bundle");
	image->attachment.name_offset = loader_image_add_string(image, &strings_len,
							 "loader_ttl_attachment");
	image->attachment.hook_name_offset = loader_image_add_string(image, &strings_len,
							      "kernel/thread_switched_in");
	image->attachment.prog_type = EBPF_PROG_TYPE_GENERIC;
	image->attachment.insns_offset = offsetof(struct loader_test_image, insns);
	image->attachment.insn_cnt = ARRAY_SIZE(loader_counter_prog);
	memcpy(image->insns, loader_counter_prog, sizeof(loader_counter_prog));
	image->insns[3].imm = (int32_t)map_id;
	loader_image_finalize(image, strings_len);
}
...
{
	return create_prog_instance(name, EBPF_PROG_TYPE_GENERIC,
...
static struct ebpf_prog *create_prog_from_insns(const char *name,
						...
						uint32_t insn_cnt)
{
	return create_prog_instance(name, EBPF_PROG_TYPE_GENERIC, insns, insn_cnt);
}
...
ZTEST(ebpf_tracing, test_hook_api_attaches_using_stable_hook_id)
{
	...
	zassert_ok(ebpf_attach_target_to_hook_id(&target, &hook_id),
		   "reverse hook translation failed");
	zassert_equal(hook_id, EBPF_HOOK_THREAD_SWITCHED_IN,
		      "reverse hook translation should recover original hook id");
	zassert_true(ebpf_hook_id_is_valid(hook_id),
		     "resolved hook id should be valid");
	zassert_true(strcmp(ebpf_hook_id_name(hook_id), "kernel/thread_switched_in") == 0,
		     "stable hook name should match expected string");
...
ZTEST(ebpf_tracing, test_loader_loads_named_hook_with_runtime_map_reloc)
{
	...
	uint32_t key = 0U;
	uint32_t *value;
	uint32_t before_manual_dispatch;
	uint32_t map_id;
	int ret;
	...
	zassert_not_null(bundle, "loaded object should expose a runtime bundle");
	map = ebpf_bundle_find_map(bundle, "dyn_counter");
	zassert_not_null(map, "runtime-loaded map should be findable by name");
	map_id = ebpf_map_get_id(map);
	zassert_not_equal(map_id, 0U, "runtime-loaded map should receive an id");

	ret = ebpf_loader_enable(handle);
	zassert_ok(ret, "loader enable failed: %d", ret);

	value = ebpf_map_lookup_elem(map, &key);
	zassert_not_null(value, "runtime-loaded map lookup failed");
	before_manual_dispatch = *value;
	ebpf_attach_target_dispatch(&target, &image, sizeof(image));
	value = ebpf_map_lookup_elem(map, &key);
	zassert_not_null(value, "runtime-loaded map lookup failed");
	zassert_equal(*value, before_manual_dispatch + 1U,
		      "runtime-loaded program should increment the dynamic map exactly once, got %u",
		      *value);

	ret = ebpf_loader_unload(handle);
	zassert_ok(ret, "loader unload failed: %d", ret);
	zassert_is_null(ebpf_map_from_id(map_id),
		       "runtime-loaded map should be destroyed on unload");
}
...
ZTEST(ebpf_tracing, test_loader_ttl_auto_unloads_attachment)
{
	struct loader_test_image image;
	struct ebpf_loader_handle *handle;
	struct ebpf_attach_target target =
		EBPF_ATTACH_TARGET_TRACING(EBPF_TRACING_ATTACH_THREAD_SWITCHED_IN);
	struct ebpf_map_spec ttl_map_spec = {
		.name = "loader_ttl_counter_map",
		.type = EBPF_MAP_TYPE_ARRAY,
		.key_size = sizeof(uint32_t),
		.value_size = sizeof(uint32_t),
		.max_entries = 1U,
	};
	struct ebpf_map *ttl_map;
	uint32_t key = 0U;
	uint32_t *value;
	uint32_t before_manual_dispatch;
	uint32_t after_ttl;
	...
	ret = ebpf_map_create(&ttl_map_spec, &ttl_map);
	zassert_ok(ret, "failed to create TTL test map: %d", ret);
	map_id = ebpf_map_get_id(ttl_map);
	zassert_not_equal(map_id, 0U, "TTL test map should have a runtime id");
	value = ebpf_map_lookup_elem(ttl_map, &key);
	zassert_not_null(value, "TTL test map lookup failed");
	*value = 0U;

	loader_build_ttl_image(&image, map_id, 20U);

	ret = ebpf_loader_load(&image, image.header.total_size, &handle);
	zassert_ok(ret, "TTL loader load failed: %d", ret);

	ret = ebpf_loader_enable(handle);
	zassert_ok(ret, "TTL loader enable failed: %d", ret);

	before_manual_dispatch = *value;
	ebpf_attach_target_dispatch(&target, &image, sizeof(image));
	zassert_equal(*value, before_manual_dispatch + 1U,
		      "TTL-loaded program should increment the external map exactly once");

	k_sleep(K_MSEC(100));
	zassert_is_null(ebpf_loader_bundle(handle),
		       "bundle should be auto-unloaded after TTL expiry");
	zassert_false(ebpf_attach_target_is_active(&target),
		      "TTL expiry should remove the attachment from the target snapshot");

	after_ttl = *value;
	ebpf_attach_target_dispatch(&target, &image, sizeof(image));
	zassert_equal(*value, after_ttl,
		      "expired loader attachment must stop dispatching after auto-unload");

	ret = ebpf_loader_unload(handle);
	zassert_ok(ret, "TTL loader handle cleanup failed: %d", ret);

	ret = ebpf_map_destroy(ttl_map);
	zassert_ok(ret, "failed to destroy TTL test map: %d", ret);
}
...
ZTEST(ebpf_tracing, test_bundle_enable_rolls_back_on_attachment_failure)
{
	struct ebpf_attach_target target =
		EBPF_ATTACH_TARGET_TRACING(EBPF_TRACING_ATTACH_THREAD_SWITCHED_IN);
	struct ebpf_bundle_attachment_spec good_spec = {
	...
	struct ebpf_bundle_attachment_spec bad_spec = {
		.name = "bad_attachment",
		.type = EBPF_PROG_TYPE_GENERIC,
		.insns = invalid_prog,
