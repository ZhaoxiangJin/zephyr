...
#include "../../../../subsys/ebpf/insn/ebpf_insn.h"
#include "../../../../subsys/ebpf/helpers/ebpf_helpers_internal.h"
#include "../../../../subsys/ebpf/prog/ebpf_prog_internal.h"

extern int64_t ebpf_vm_exec(const struct ebpf_prog_image *prog, void *ctx_data,
			    uint32_t ctx_size);

static struct ebpf_prog_image make_test_prog(const char *name,
					   const struct ebpf_insn *insns,
					   uint32_t insn_cnt)
{
	return (struct ebpf_prog_image) {
		.name = name,
...
ZTEST(ebpf_vm, test_mov_add_exit)
{
	...
	struct ebpf_prog_image test_prog = make_test_prog("test_prog", prog_insns,
...
ZTEST(ebpf_vm, test_sub)
{
	...
	struct ebpf_prog_image test_sub = make_test_prog("test_sub", prog_insns,
						 ARRAY_SIZE(prog_insns));
...
ZTEST(ebpf_vm, test_mov_reg)
{
	...
	struct ebpf_prog_image test_movreg = make_test_prog("test_movreg", prog_insns,
						    ARRAY_SIZE(prog_insns));
...
ZTEST(ebpf_vm, test_mul)
{
	...
	struct ebpf_prog_image test_mul = make_test_prog("test_mul", prog_insns,
						 ARRAY_SIZE(prog_insns));
...
ZTEST(ebpf_vm, test_div_by_zero)
{
	...
	struct ebpf_prog_image test_div0 = make_test_prog("test_div0", prog_insns,
						  ARRAY_SIZE(prog_insns));
...
ZTEST(ebpf_vm, test_bitwise)
{
	...
	struct ebpf_prog_image test_bits = make_test_prog("test_bits", prog_insns,
						  ARRAY_SIZE(prog_insns));
...
ZTEST(ebpf_vm, test_jeq)
{
	...
	struct ebpf_prog_image test_jeq = make_test_prog("test_jeq", prog_insns,
						 ARRAY_SIZE(prog_insns));
...
ZTEST(ebpf_vm, test_jne)
{
	...
	struct ebpf_prog_image test_jne = make_test_prog("test_jne", prog_insns,
						 ARRAY_SIZE(prog_insns));
...
ZTEST(ebpf_vm, test_stack_memory)
{
	...
	struct ebpf_prog_image test_stack = make_test_prog("test_stack", prog_insns,
						   ARRAY_SIZE(prog_insns));
...
ZTEST(ebpf_vm, test_context_read)
{
	...
	struct ebpf_prog_image test_ctx_prog = make_test_prog("test_ctx", prog_insns,
						      ARRAY_SIZE(prog_insns));
...
ZTEST(ebpf_vm, test_call_helper)
{
	...
	struct ebpf_prog_image test_call = make_test_prog("test_call", prog_insns,
						  ARRAY_SIZE(prog_insns));
...
ZTEST(ebpf_vm, test_unknown_opcode)
{
	...
	struct ebpf_prog_image test_bad_op = make_test_prog("test_bad_op", prog_insns,
						    ARRAY_SIZE(prog_insns));
...
ZTEST(ebpf_vm, test_runtime_bounds_check_rejects_wrapped_region)
{
	...
	struct ebpf_prog_image prog = make_test_prog("test_wrapped_region",
						    prog_insns,
						    ARRAY_SIZE(prog_insns));
...
ZTEST(ebpf_vm, test_runtime_rejects_context_write)
{
	...
	struct ebpf_prog_image prog = make_test_prog("ctx_write_runtime_reject",
						    prog_insns,
						    ARRAY_SIZE(prog_insns));
